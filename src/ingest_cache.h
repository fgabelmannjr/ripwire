...
constexpr std::uint32_t kParserVer    = 82;           // bump on any grammar/.scm/extraction change
                                                      // 82 = 2026-09-07 (+Astro, test/astrocheck.sh): `.astro`
                                                      //    joins kLangTable on the TypeScript grammar behind the
                                                      //    ingest_astro.h region blanker, and gains a component
                                                      //    symbol plus template-tag call references per file. A
                                                      //    v81 blob on an Astro-bearing tree has NO rows for those
                                                      //    files at all (they were unsupported-ext) → reject. The
                                                      //    record shape did not change, so only parserVer moves —
                                                      //    the CUDA (37) precedent exactly.
