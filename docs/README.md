...
| **[`plans/`](plans/)** | Maintainers picking up a multi-phase round | One plan per round with a phase ledger at the top: the measured rationale, the decided fix shape per file, the gate, the siblings, and what is open — so a later phase starts from evidence, not memory. Currently `astro-support.md`. |
